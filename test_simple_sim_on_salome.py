...
def run_simple_sim():
    ...
    fluid_id = np.abs(correspondance_dict["fluid"])
    inlet_id = np.abs(correspondance_dict["inlet"])
    outlet_id = np.abs(correspondance_dict["outlet"])
    walls_id = np.abs(correspondance_dict["walls"])
    ...
    my_sim.materials = F.Material(id=fluid_id, D_0=2, E_D=0)  # fails unless D = 1
