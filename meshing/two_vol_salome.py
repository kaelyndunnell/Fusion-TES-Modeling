...
def build_and_export(
    ...
    if r1 >= float(bend_radius):
        print(
            "[SKIP] tube_radius ({}) must be < bend_radius ({})".format(r1, bend_radius)
        )
        return None
    ...
    two_vol = geompy.MakePartition(
        [Pipe_1, Cut_1, Cut_2], [], [], [], geompy.ShapeType["SOLID"], 0, [], 0
    )
    ...
    axial_edges = [e for e in all_edges if geompy.BasicProperties(e)[0] > r3 * 2]
    ...
    # --- Mesh ---
    mesh_name = "two_vol_{}_{:.2f}_{:.2f}".format(tube_radius, bend_radius, length)
    ...
    smesh.SetName(
        Mesh_1.GroupOnGeom(interfaces, "interfaces", SMESH.FACE), "interfaces"
    )
...
if __name__ == "__main__":
    ...
    print(
        "--- tube_r={} bl_t={} wall_t={} bend_r={} length={} ---".format(
    ...
    try:
        ...
        print(
            "[ERROR] {},{},{},{},{}: {}".format(tube_r, bl_t, wall_t, bend_r, length, e)
        )
