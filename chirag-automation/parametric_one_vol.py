...
MESH_SCRIPT = os.path.join(SCRIPT_DIR, 'one_vol_salome.py')
UNV_DIR     = os.path.join(SCRIPT_DIR, 'meshes/bl/one_vol/UNV')
...
def mesh_utube(tube_r, bl_t, bend_r, length, unv_dir):
    env = os.environ.copy()
    ...
    print(f"Meshing tube_r={tube_r} bl_t={bl_t} bend_r={bend_r} length={length}...")
    ...
    if result.returncode != 0:
        print(result.stderr)
        raise RuntimeError(f"Meshing failed: {tube_r},{bl_t},{bend_r},{length}")

    mesh_path = os.path.join(
        unv_dir,
        f"one_vol_{float(tube_r)}_{float(bl_t)}_{float(bend_r)}_{float(length)}.unv"
    )
...
params = list(itertools.product(TUBE_RADII, BL_THICK, BEND_RADII, LENGTHS))
print(f"Total cases: {len(params)}")

for tube_r, bl_t, bend_r, length in params:
    if tube_r >= bend_r:
    ...
    if tube_r - bl_t <= 0:
        ...
        continue
    try:
        mesh_file = mesh_utube(tube_r, bl_t, bend_r, length, UNV_DIR)
        run_fem(mesh_file, tube_r=tube_r, bl_t=bl_t,
