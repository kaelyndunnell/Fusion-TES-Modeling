...
def build_and_export(tube_radius, bl_thickness, bend_radius, length, unv_dir,
    ...
    # --- Mesh ---
    mesh_name = "one_vol_{}_{}_{}_{}".format(
        tube_radius, bl_thickness, bend_radius, length)
...
if __name__ == '__main__':
    tube_r  = float(os.environ.get('TUBE_R',    5.0))
    bl_t    = float(os.environ.get('BL_T',      0.2))
    bend_r  = float(os.environ.get('BEND_R',   15.0))
    length  = float(os.environ.get('LENGTH',   40.0))
    unv_dir = os.environ.get('UNV_DIR',
              '/home/ckhurana/Fusion-TES-Modeling/chirag-automation/meshes/bl/one_vol/UNV')
