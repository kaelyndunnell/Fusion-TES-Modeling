...
def build_and_export(tube_radius, bl_thickness, wall_thickness, bend_radius, length, unv_dir,
    ...
    # --- Mesh ---
    mesh_name = "u_tube_{}_{}_{}_{}_{}".format(
        tube_radius, bl_thickness, wall_thickness, bend_radius, length)
    ...
    # --- Only the 5 groups OpenFOAM needs ---
    smesh.SetName(Mesh_1.GroupOnGeom(fluid,  'fluid',  SMESH.VOLUME), 'fluid')
    smesh.SetName(Mesh_1.GroupOnGeom(wall,   'wall',   SMESH.VOLUME), 'wall')
...
if __name__ == '__main__':
    ...
    bl_t    = float(os.environ.get('BL_T',      0.000575))
    wall_t  = float(os.environ.get('WALL_T',    0.0004))
    bend_r  = float(os.environ.get('BEND_R',   0.5))
    length  = float(os.environ.get('LENGTH',   8.0))
    unv_dir = os.environ.get('UNV_DIR', '/home/ckhurana/Fusion-TES-Modeling/chirag-automation/meshes/bl/two_vol/debugging')
    ...
    try:
        build_and_export(tube_r, bl_t, wall_t, bend_r, length, unv_dir,
                         n_segments=n_seg,
                         growth_rate=growth,
