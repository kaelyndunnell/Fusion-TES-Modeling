        ...
        try:
            ...
            subprocess.run(
                [
                    ...
                    "-m",
                    f"{parent_dir}/meshing/parametric_meshes",  # meshing folder
                    "-r",
                    f"{bend_radius}",  # bend radius (m)
                    "-l",
                    f"{length}",  # pipe length (m)
                ]
            )
            ...
            # PIPE_PATHS
            PIPE_NEW_FOLDER = f"{parent_dir}/openfoam/velocity_parametrization/lipb/pipe_{v_in:.2f}m_s_r{bend_radius:.2f}_l{length:.2f}"
            if v_in <= 0.52:
